...
def test_build_case_index_summarizes_cases(tmp_path):
    ...
    assert all("fingerprint_count" in case for case in index["cases"])
    assert all("rich_header_entry_count" in case for case in index["cases"])
    assert all("version_info_count" in case for case in index["cases"])
    ...
    assert all("xref_count" in case for case in index["cases"])
